...
4. Le program envoie la photo à l'IA : l'IA est comme un expert qui a appris en regardant beaucoup de photos auparavant.
5. L'IA renvoie sa meilleure proposition (par ex. "héron") et un pourcentage qui indique à quel point elle est sûre.
6. Selon ce pourcentage, le program classe la photo et l'enregistre dans un dossier adapté :
...
- L'IA apprend à partir d'examples. Si une espèce est rarement présente dans les examples ou si la photo est mauvaise, l'IA peut mal interpréter.
...
- Analyze en arrière-plan : l'ordinateur peut traiter la photo sans bloquer la vidéo, ce qui évite que la fenêtre se fige.
