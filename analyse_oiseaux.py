# ============================================================================
# ANALYZE OISEAUX - Script d'analyse des prédictions YOLOv5 pour classification d'oiseaux
# Ce script lance des prédictions avec un modèle YOLOv5 entraîné et analyze les résultats
# ============================================================================
...
# Répertoire racine du project YOLOv5
...
def load_local_env_file(env_file: Path = LOCAL_ENV_FILE) -> None:
    ...
    Le format attendu est KEY=VALUE, avec commentaires # et valeurs entre guillemets acceptées. Les variables déjà
    définies dans l'environnement ne sont pas écrasées.
    """
...
# Synchronization Azure optionnelle (activée seulement si les variables d'environnement sont renseignées)
...
def ensure_dependency(module_name: str, pip_name: str | None = None) -> None:
    ...
    # Utilize le nom pip fourni ou se replie sur le nom du module
...
# ============================================================================
# IMPORTS TARDIFS - Dépendances externes guarantees d'être disponibles
# ============================================================================
...
class PredictionRecord:
    """Représente une seule prédiction de classification d'image YOLOv5. Contient tous les scores de confiance et le
    statut post-traitement.
    """
...
# ============================================================================
# FUNCTIONS UTILITAIRES - Helpers pour affichage et traitement des données
# ============================================================================


def console_text(text: str, color: str = "", bright: bool = False) -> str:
    """Format du texte avec couleur et style pour l'affichage en console.
...
def normalize_label(value: str) -> str:
    """Normalize un label pour la comparison robuste entre dossier et prédiction. Supprime accents, escapes, caractères
    spéciaux, convertit en minuscules.
    ...
    """
    # Normalize les accents (décompose les caractères accentués)
    normalized = unicodedata.normalize("NFKD", value)
    # Supprime les diacritiques (accents)
    normalized = "".join(character for character in normalized if not unicodedata.combining(character))
    # Minuscules et supprime les escapes
    normalized = normalized.lower().strip()
    # Replace tirets et escapes par underscores
...
def build_run_name(source: Path) -> str:
    """Génère un nom unique court pour les outputs YOLOv5. Utilize le nom du fichier normalisé + hash SHA1 du chemin
    complete.
    ...
    """
    # Utilize le stem (nom sans extension) ou le nom du dossier
...
def get_recording_subfolder(record: PredictionRecord) -> str:
    """Détermine le sous-dossier de sauvegarde en function du statut métier.
...
def build_recording_name(source_path: Path, record: PredictionRecord) -> str:
    ...
    digest = hashlib.sha1(
        f"{source_path.resolve()}|{record.status}|{record.top1_class}|{timestamp}".encode()
    ).hexdigest()[:8]
    return f"{source_base}_{status_base}_{timestamp}_{digest}.jpg"


def save_analyzed_image(source_path: Path, record: PredictionRecord) -> Path | None:
...
def transferer_donnees_azure(
    chemin_image_locale: Path, espece_oiseau: str, score_confiance: float, statut: str
) -> bool:
    """Envoie une image analysée sur Azure Blob Storage et l'alerte associée sur Azure IoT Hub.

    L'envoi est volontairement optionnel: il ne s'active que si les variables d'environnement Azure sont renseignées.
    ...
    if not AZURE_STORAGE_CONN or not AZURE_IOT_HUB_CONN:
        print(
            console_text(
                "[Azure] Connexions manquantes: envoi désactivé (variables d'environnement non renseignées).",
                Fore.YELLOW,
                bright=True,
            )
        )
    ...
    try:
        print(
            console_text(
                f"[Azure] Téléversement de {nom_fichier_azure} vers le container '{AZURE_BLOB_CONTAINER}'...",
                Fore.CYAN,
                bright=True,
            )
        )
        ...
        print(console_text("[Azure] Synchronization terminée avec succès.", Fore.GREEN, bright=True))
        return True
    except Exception as exc:
...
def find_audio_file(bird_class: str) -> Path | None:
    """Recherche et retourne un fichier audio aléatoire correspondent à un oiseau détecté.
    ...
    """
    # Normalize le nom de classe pour la comparison
    normalized_class = normalize_label(bird_class)

    # Récupère le dossier audio correspondent
    audio_folder_name = CLASS_TO_AUDIO_FOLDER.get(normalized_class)
    if audio_folder_name is None:
        print(
            console_text(f"[AUDIO] Classe '{bird_class}' non mappée vers un dossier audio.", Fore.YELLOW, bright=True)
        )
        return None
    ...
    if not audio_folder.exists() or not audio_folder.is_dir():
        print(
            console_text(
                f"[AUDIO] Dossier audio introuvable pour '{bird_class}': {audio_folder}", Fore.YELLOW, bright=True
            )
        )
        return None

    # Cherche tous les fichiers MP3 du dossier
    audio_files = [f for f in audio_folder.iterdir() if f.is_file() and f.suffix.lower() in AUDIO_EXTENSIONS]

    if not audio_files:
        print(
            console_text(
                f"[AUDIO] Aucun fichier MP3 trouvé pour '{bird_class}' dans {audio_folder}", Fore.YELLOW, bright=True
            )
        )
        return None
...
def play_audio(audio_path: Path | None) -> bool:
    """Joue un fichier audio en ouvrant le lecteur système par défaut (Windows). La function est bloquante seulement si
    le lecteur reste ouvert.
    ...
    try:
        ...
        if sys.platform == "win32":
            ...
            return True
        # Sous Linux, utilize 'xdg-open'
        elif sys.platform == "linux":
            subprocess.Popen(["xdg-open", str(audio_path)])
            print(console_text(f"[AUDIO] Lecture lancée: {audio_path.name}", Fore.GREEN, bright=True))
            return True
        # Sous macOS, utilize 'open'
...
def play_bird_audio(bird_class: str) -> bool:
    """Cherche et joue le son d'un oiseau détecté. Gère l'ensemble du processus: recherche du fichier, sélection
    aléatoire et lecture.
...
# ============================================================================
# FUNCTION PRINCIPALE D'INFÉRENCE - Exécution du modèle YOLOv5
# ============================================================================
...
def run_yolov5_prediction(source: Path, save_outputs: bool = True) -> list[PredictionRecord]:
    ...
    # ========================================================================
    # POST-TRAITEMENT - Applique les seuils de confiance et normalize
    # ========================================================================
    for rec in records:
...
def print_single_image_result(record: PredictionRecord) -> None:
    """Affiche un résumé console propre pour l'analyse d'une seule image. Affiche la classe prédite, la confiance, le
    statut et tous les scores détaillés.
...
# ============================================================================
# VISUALIZATION GRAPHIQUE - Graphique de confiance pour une image
# ============================================================================
...
def plot_single_image(record: PredictionRecord, output_dir: Path) -> Path:
    ...
    ax.set_title(f"Analyze: {record.image_path.name}", pad=12, weight="bold")
...
# ============================================================================
# VISUALIZATION GRAPHIQUE - Résumé statistique du dossier
# ============================================================================
...
def plot_folder_summary(
    ...
    """Crée un graphique professionnel avec le résumé complete du dossier analysé. Génère 2 figures: répartition
    BDD/INCERTITUDE/autre, et réussite/échec.
    ...
    _wedges, _texts = ax_pie.pie(
        statuses, labels=labels, colors=colors_pie, startangle=90, wedgeprops={"width": 0.45, "edgecolor": "white"}
    )
    ...
    bar_height = 0.8
    # Barre vert pour réussite + rogue pour échec
    ...
    ax2.text(
        success_pct / 2 if success_pct else 2,
        0,
        f"{success_pct:.1f}%",
        ha="center",
        va="center",
    ...
    ax2.text(
        success_pct + failure_pct / 2 if failure_pct else max(success_pct + 2, 2),
        0,
        f"{failure_pct:.1f}%",
        ha="center",
        va="center",
...
def write_folder_summary_json(
    ...
    """Sauvegarde un résumé complete de l'analyse en fichier JSON. Inclut les statistiques globales et la matrice de
    confusion.
    ...
    data = {
        ...
        "confusions": [{"true": t, "pred": p, "count": c} for (t, p), c in confusion_counts.most_common()],
    }
...
# ============================================================================
# ANALYZE SIMPLE - Analyze d'une seule image
# ============================================================================


def analyze_single_image(image_path: Path) -> None:
    """Analyze une seule image: lance la prédiction, affiche les résultats, joue le son approprié et génère un
    graphique.
    ...
    Args:
        image_path: Chemin vers l'image à analyzer
    ...
    # Sauvegarde l'image dans le dossier correspondent au statut détecté
    ...
    if record.status == "BDD":
        ...
        print(
            console_text("[AUDIO] Statut INCERTITUDE détecté - lecture du signal d'alerte...", Fore.YELLOW, bright=True)
        )
    ...
    if saved_image_path is not None:
        ...
        try:
            ...
            if hasattr(azure_transfer, "transferer_fichiers_azure"):
                try:
                    azure_transfer.transferer_fichiers_azure(
                        saved_image_path, record.top1_class, record.top1_score, record.status
                    )
                except Exception as e:
                ...
                # If the function is not present, fallback to legacy behavior
                transferer_donnees_azure(saved_image_path, record.top1_class, record.top1_score, record.status)
        except Exception:
    ...
    if record.status == "HORS_BDD":
        print(
            console_text(
                "Attention: cet oiseau ne semble pas appartenir aux classes du dataset (AUTRE).",
                Fore.YELLOW,
                bright=True,
            )
        )
...
# ============================================================================
# ANALYZE DOSSIER - Analyze récursive d'un dossier d'images
# ============================================================================


def analyze_folder(folder_path: Path) -> None:
    """Analyze toutes les images d'un dossier (récursivement). Compare les prédictions aux étiquettes (noms des
    sous-dossiers). Une prédiction en INCERTITUDE est comptée comme une réussite pour le bilan. Génère des
    graphiques et un fichier JSON avec les résultats détaillés.
    ...
    with tempfile.TemporaryDirectory(prefix="analyse_oiseaux_") as temp_dir:
        ...
        for image_path, record in tqdm(list(zip(image_files, records)), desc="Analyze des images", unit="image"):
    ...
    # Normalize les clés de status pour l'affichage (HORS_BDD -> autre)
...
def analyze_camera_stream(camera_index: int = 0) -> None:
    """Lance une boucle de capture depuis la camera et analyze les captures à la demande.
    ...
    print("  - c ou escape : capturer et analyzer la frame courante")
    print("  - q ou Echap  : quitter")

    camera = open_camera(camera_index)
    temp_dir = Path(tempfile.mkdtemp(prefix="analyse_oiseaux_camera_"))

    # Event thread-safe pour indiquer si une analyze est en cours
    ...
    if vector_enabled:
        try:
            ...
            print(
                console_text(
                    f"[VECTOR] Index chargé ({len(faiss_mapping)} items). Analyze continue activée.",
                    Fore.MAGENTA,
                    bright=True,
                )
            )
            ...
            print(
                console_text(
                    f"[VECTOR] Impossible d'activer l'analyse vectorielle continue: {exc}", Fore.YELLOW, bright=True
                )
            )
    ...
    try:
        while True:
            ...
            # Overlay status: affiche si une analyze est en cours ou temps restant avant capture
            now = time.time()
            in_progress = analysis_event.is_set()
            cooldown_remaining = max(0.0, last_capture_time + capture_cooldown - now)
            if in_progress:
                status_text = "ANALYZE EN COURS..."
                color = (0, 200, 200)
            elif cooldown_remaining > 0.0:
                status_text = f"Prêt dans {math.ceil(cooldown_remaining)}s"
                color = (0, 200, 200)
            else:
                status_text = "Appuyez 'c' ou Escape pour capturer"
                color = (200, 200, 200)

            cv2.putText(
                frame, status_text, (10, frame.shape[0] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2, cv2.LINE_AA
            )
            cv2.imshow("Analyze oiseaux - camera", frame)
            ...
            if key in (ord("c"), ord(" ")):
                ...
                if analysis_event.is_set():
                    print(console_text("[CAMERA] Analyze en cours, veuillez patienter...", Fore.YELLOW, bright=True))
                else:
                    ...
                    def _analyze_async(path: Path):
                        try:
                            ...
                            print(
                                console_text(f"[CAMERA] Erreur durant l'analyse asynchrone: {e}", Fore.RED, bright=True)
                            )
                        finally:
                            try:
                                path.unlink()
            ...
            # Analyze vectorielle continue (périodique)
            try:
...
def ask_user_choice() -> str:
    ...
    Returns:
        "1" pour analyzer une seule image, "2" pour analyzer un dossier, "3" pour utiliser la camera
    """
    print(console_text("\n=== Analyze oiseaux YOLOv5 ===", Fore.MAGENTA, bright=True))
    print("1. Analyzer une seule image")
    print("2. Analyzer un dossier complete d'images")
    print("3. Analyzer depuis la camera")
    choice = input("Votre choix (1/2/3) : ").strip()
    while choice not in {"1", "2", "3"}:
        print(console_text("Choix invalid. Réessaie avec 1, 2 ou 3.", Fore.RED, bright=True))
        choice = input("Votre choix (1/2/3) : ").strip()
    return choice
...
def ask_camera_index() -> int:
    ...
    Returns:
        Index caméra saisi, ou 0 si la saisie est vide/invalid
    """
    ...
    try:
        ...
        print(console_text("Index invalid, utilization de la camera 0.", Fore.YELLOW, bright=True))
        return 0
...
# ============================================================================
# INITIALIZATION - Vérifications du project
# ============================================================================


def ensure_project_environment() -> None:
    """Vérifie que l'environnement du project est correct: - Crée le répertoire de résultats - Vérifie que les poids du
    modèle existent - Vérifie que le script YOLOv5 existe.
...
def main() -> None:
    """Point d'entrée principal du script. Gère le flux d'exécution: 1. Vérifie l'environnement 2. Demande à
    l'utilisateur son choix (image ou dossier) 3. Lance l'analyse appropriée 4. Gère les erreurs et affiche
    les messages.
    """
    try:
        ...
        if choice == "1":
            # Analyze d'une seule image
            image_path = ask_path("Chemin complete de l'image : ")
            analyze_single_image(image_path)
        elif choice == "2":
            # Analyze d'un dossier
            folder_path = ask_path("Chemin complete du dossier : ")
            analyze_folder(folder_path)
        else:
            # Analyze depuis la camera
