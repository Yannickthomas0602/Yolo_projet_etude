# Analyze vectorielle expliquée — fonctionnement concret

Ce document explique, pas à pas et de manière concrète, comment fonctionne l'analyse vectorielle utilisée dans ce project (CLIP pour les embeddings + FAISS pour l'indexation). Il montre ce qui se passe depuis l'image d'entrée jusqu'à la décision métier.

## 1 — Vue d'ensemble du pipeline

1. Prétraitement de l'image (resize, conversion RGB, normalization).
2. Encodage par un modèle d'embeddings (ex: CLIP) → vecteur $\\mathbf{v} \\in \\mathbb{R}^d$.
3. Normalization du vecteur (L2) pour obtenir $\\hat{\\mathbf{v}} = \\mathbf{v} / \\|\\mathbf{v}\\|$.
4. Recherche des K plus proches voisins dans un index FAISS (inner product sur vecteurs normalisés ⇒ cosinus).
5. Récupération des chemins/métadonnées correspondents via `mapping.json`.
6. Combination des scores de similarité avec la sortie du classifieur (YOLOv5) pour la décision finale.
...
- Comparison stable entre éléments.
- FAISS optimize l'inner product pour des recherches rapides (IndexFlatIP, ou indexes ANN plus avancés).

## 3 — Étapes concrètes côté code (example simplifié)
...
Examples de valeurs recommandées (à calibrer) :
...
- 0.3–0.6 : resemblance faible → utile comme signal complémentaire mais non définitif.
...
- Examples de stratégie : calculer un embedding toutes les N secondes, ou quand le classifieur indique un changement de classe.
...
3. Choisir `SIM_THRESH_LOW` et `SIM_THRESH_HIGH` en function des distributions (par ex. seuil à 95e percentile des scores inter-classe).

## 10 — Resources rapides

- Code du project : [yolov5/vector_index.py](vector_index.py)
- Documentation FAISS : https://github.com/facebookresearch/faiss
- CLIP (OpenAI) : https://github.com/openai/CLIP

---

Si tu veux, j'intègre ces règles directement dans `analyse_oiseaux.py` (functions utilitaires + seuils config) et je lance un petit script de calibration sur `enregistrements/`. Tu veux que je le fasse ?
