...
1. analyze d'une image unique,
2. analyze d'un dossier d'images,
3. analyze en direct depuis la camera de l'ordinateur.

Le but est de garder la logique actuelle du project, deja documentee dans [GUIDE_OISEAUX.md](GUIDE_OISEAUX.md), [LANCER_IA_OISEAUX.md](LANCER_IA_OISEAUX.md) et [JOURNAL_PROJET_OISEAUX.md](JOURNAL_PROJET_OISEAUX.md), tout en ajoutant un mode temps reel simple a utiliser et facile a maintenir.

## 1. Objectif fonctionnel

Le script doit proposer un choix clair a l'utilisateur au lancement :

- analyzer une image locale,
- analyzer tout un dossier d'images,
...
- meme lecture audio pour une image unique si le project la conserve.
...
En pratique, le mode camera ne doit pas dupliquer la logique de classification. Il doit seulement fournir une image courante au motor d'analyse existent.
...
La function centrale a reutiliser est [run_yolov5_prediction](analyse_oiseaux.py), qui prend actuellement un chemin vers une image ou une liste d'images. Le mode camera peut s'appuyer dessus en ecrivant chaque frame dans un fichier temporaire avant de lancer l'analyse.

## 4. Architecture proposee pour la camera

### 4.1 Nouvelles functions a ajouter

Tu peux ajouter trois functions principales :
...
Si tu veux garder le code plus compact, une seule function de haut niveau peut aussi suffire :
...
### 4.2 Dependences usuelles

Le mode camera necessite en pratique OpenCV.

Le project utilize deja des dependences dynamiques dans [analyse_oiseaux.py](analyse_oiseaux.py), donc tu peux :

- ajouter opencv-python aux dependences si ce n'est pas deja present,
...
La camera doit etre ouverte avec l'index 0 par default, ce qui correspond generalement a la camera principale.

Il peut etre utile de prevoir un parameter optionnel si l'ordinateur possede plusieurs cameras :
...
- analyzer une frame sur plusieurs,
...
```text
1. Analyzer une image
2. Analyzer un dossier
3. Analyzer depuis la camera
0. Quitter
```
...
### Option A - Analyze ponctuelle a la demande
...
### Option B - Analyze continue avec affichage

Le principe : la camera tourne en boucle et chaque frame ou sous-ensemble de frames est analyze.
...
Pour ce project, l'option A est la plus robuste pour une premiere version. L'option B peut venir ensuite si besoin.
...
La lecture audio est adapted a une image unique. En mode camera, il faut eviter de relancer le son en boucle sur la meme espece si la camera analyze plusieurs frames successives.
...
Le mode camera sera plus fluide si tu utilizes :

- un modele leger,
- une resolution moderee,
- une frequency d'inference limitee,
...
- en mode image : analyze d'un seul fichier,
- en mode dossier : analyze recursive et bilan global,
- en mode camera : ouverture de la camera, prediction sur demande ou en continu, arret avec une touche clavier.

Example d'usage cible :
...
Le nom du dossier BDD dépend de la classe détectée. Par example :
...
1. Ajouter la dependence OpenCV si elle manque.
2. Ajouter le choix camera dans le menu ou dans argparse.
3. Creer une function de capture camera.
...
Avant de considerer le mode camera comme determine, il faut verifier :
...
Si tu veux rendre cette fonctionnalité plus visible dans la documentation existante, ajoute ensuite un lien vers ce document depuis :
...
## 14. Analyze vectorielle continue (CLIP + FAISS)

Principe :

- On utilize CLIP (modèle de représentation visuelle) pour convertir une image en un vecteur d'embedding haute-dimension.
...
1. Active l'environnement Python du project.
